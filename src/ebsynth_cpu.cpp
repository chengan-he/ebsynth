        ...
        for (int x = 0; x < target.width(); x++) {
            ...
            for (int py = -r; py <= +r; py++)
                for (int px = -r; px <= +r; px++) {
                    if (
                        x + px >= 0 && x + px < NNF.width() && y + py >= 0 && y + py < NNF.height()) {
                        const V2i n = NNF(x + px, y + py) - V2i(px, py);

                        if (
                            n[0] >= 0 && n[0] < source.width() && n[1] >= 0 && n[1] < source.height()) {
...
Vec<N, T> sampleBilinear(const Array2<Vec<N, T>>& I, float x, float y)
{
    ...
    return Vec<N, T>((1.0f - s) * (1.0f - t) * Vec<N, float>(I(clamp(ix, 0, I.width() - 1), clamp(iy, 0, I.height() - 1))) + (s) * (1.0f - t) * Vec<N, float>(I(clamp(ix + 1, 0, I.width() - 1), clamp(iy, 0, I.height() - 1))) + (1.0f - s) * (t)*Vec<N, float>(I(clamp(ix, 0, I.width() - 1), clamp(iy + 1, 0, I.height() - 1))) + (s) * (t)*Vec<N, float>(I(clamp(ix + 1, 0, I.width() - 1), clamp(iy + 1, 0, I.height() - 1))));
};
...
struct PatchSSD_Split {
                     ...
                     const float ebest)
    {
        ...
        if (tx - r >= 0 && tx + r < targetStyle.width() && ty - r >= 0 && ty + r < targetStyle.height()) {
                ...
                A2i&        Omega)
{
    ...
    const float omegaBest = (float(sizeA(0) * sizeA(1)) / float(sizeB(0) * sizeB(1))) * float(patchWidth * patchWidth);
                   ...
                   void*  outputImageData)
{
    ...
    if (numStyleChannels >= 1 && numStyleChannels <= EBSYNTH_MAX_STYLE_CHANNELS && numGuideChannels >= 1 && numGuideChannels <= EBSYNTH_MAX_GUIDE_CHANNELS) {
